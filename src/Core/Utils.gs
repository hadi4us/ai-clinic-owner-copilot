...
function writeAudit_(actorId, role, action, resource, metadata) {
  ...
  const safeMetadata = Object.assign({}, metadata || {}, {
    ...
    environment: getScriptProperty_('ENVIRONMENT_NAME', 'production'),
    release_label: getScriptProperty_('RELEASE_LABEL', ''),
    release_apps_script_version: getScriptProperty_('RELEASE_APPS_SCRIPT_VERSION', ''),
  });
  appendObjects_('AUDIT_LOG', [{
    ...
    target_id: safeMetadata.targetId || safeMetadata.importId || safeMetadata.tenantId || safeMetadata.email || '',
    before_json: safeMetadata.before ? JSON.stringify(safeMetadata.before) : '',
    after_json: JSON.stringify(safeMetadata),
    ip_or_channel: 'gas',
    created_at: new Date(),
  }]);
}
