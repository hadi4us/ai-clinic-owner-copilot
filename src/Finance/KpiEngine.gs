...
function computePocKpisNoLock_(tenantId, clinicId, period) {
  ...
  const now = new Date();
  const visits = getScopedPeriodRows_('KUNJUNGAN', tenantId, clinicId, 'visit_date', period).filter(r => String(r.status || 'completed') !== 'cancelled');
  ...
  const procedures = getScopedPeriodRows_('TINDAKAN', tenantId, clinicId, 'procedure_date', period).filter(r => String(r.status || 'active') !== 'cancelled');
  const prescriptions = getScopedPeriodRows_('RESEP', tenantId, clinicId, 'prescription_date', period).filter(r => String(r.status || 'active') !== 'cancelled');
  const expenses = getScopedPeriodRows_('BIAYA', tenantId, clinicId, 'expense_date', period).filter(r => String(r.status || 'paid') !== 'cancel');
  const bpjsClaims = getScopedPeriodRows_('BPJS_KLAIM', tenantId, clinicId, ['claim_period', 'service_month', 'claim_date'], period);
  const inventory = getScopedPeriodRows_('PERSEDIAAN', tenantId, clinicId, 'stock_date', period);
...
function computePocDailyKpis_(tenantId, clinicId, period, revenues, prescriptions, procedures, expenses, visits, bpjsClaims, inventory, taxes, traceStatus, dataStatus) {
  const dates = {};
  const byDate = {
    revenues: groupRowsByIsoDate_(revenues, 'transaction_date', dates),
    procedures: groupRowsByIsoDate_(procedures, 'procedure_date', dates),
    prescriptions: groupRowsByIsoDate_(prescriptions, 'prescription_date', dates),
    expenses: groupRowsByIsoDate_(expenses, 'expense_date', dates),
    visits: groupRowsByIsoDate_(visits, 'visit_date', dates),
    bpjsClaims: groupRowsByIsoDate_(bpjsClaims, 'claim_date', dates),
    inventory: groupRowsByIsoDate_(inventory, 'stock_date', dates),
    taxes: groupRowsByIsoDate_(taxes, 'created_at', dates),
  };
  const dailyRows = Object.keys(dates).sort().map(date => {
    const dayRevenues = byDate.revenues[date] || [];
    const dayProcedures = byDate.procedures[date] || [];
    const dayPrescriptions = byDate.prescriptions[date] || [];
    const dayExpenses = byDate.expenses[date] || [];
    const dayVisits = byDate.visits[date] || [];
    const dayBpjsClaims = byDate.bpjsClaims[date] || [];
    const dayInventory = byDate.inventory[date] || [];
    const dayTaxes = byDate.taxes[date] || [];
    ...
    return {
      ...
      bpjs_claim_amount: sumByField_(dayBpjsClaims, 'claim_amount'),
      bpjs_outstanding_amount: sumByField_(dayBpjsClaims, 'outstanding_amount'),
      inventory_value: sumByField_(dayInventory, 'stock_value'),
      expired_stock_value: dayInventory.filter(r => String(r.status || '').toLowerCase() === 'expired').reduce((sum, r) => sum + asNumber_(r.stock_value, 0), 0),
      tax_payable: sumByField_(dayTaxes, 'tax_payable'),
      data_status: dataStatus,
      trace_status: traceStatus,
...
function groupRowsByIsoDate_(rows, dateField, datesOut) {
  return (rows || []).reduce(function(acc, row) {
    const date = toIsoDateString_(row[dateField]);
    if (!date) return acc;
    if (!acc[date]) acc[date] = [];
    acc[date].push(row);
    if (datesOut) datesOut[date] = true;
    return acc;
  }, {});
}
...
function getFinanceSummary(tenantId, clinicId, period) {
  assertTenantScope_(tenantId, clinicId);
  return getScopedPeriodRows_('KPI_BULANAN', tenantId, clinicId, 'period', period).find(r => toPeriodString_(r.period) === period) || null;
}
...
function getPreviousMonthlyKpi_(tenantId, clinicId, period) {
  ...
  return getScopedPeriodRows_('KPI_BULANAN', tenantId, clinicId, 'period', prevPeriod).find(r => toPeriodString_(r.period) === prevPeriod) || null;
}
