...
A versioned deployment was updated from the `hadi4us@gmail.com`-authorized clasp user on 2026-06-14.

- Apps Script ID: `1-2IlwXdJ6jih3KRgO5cOHQon2zDnYGEq06gyXAa37wPGk4KE99Tgoaoy`
- Version: `38` — `Normalize spreadsheet period handling 2026-06-14`
...
- Readiness status: pending authenticated browser/user verification. Anonymous fetch redirects to Google sign-in, as expected for non-anonymous pilot access. `clasp run readinessCheck` currently returns `Script function not found. Please make sure script is deployed as API executable`, so browser-based authenticated verification is still required.
